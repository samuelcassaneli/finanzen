...
export default async function DashboardPage() {
  ...
  const inflow = monthTxs.filter((t) => t.type === "inflow").reduce((s, t) => s + Number(t.amount), 0);
  const outflow = monthTxs.filter((t) => t.type === "outflow").reduce((s, t) => s + Math.abs(Number(t.amount)), 0);
      ...
      <main className="mx-auto max-w-5xl px-6 py-8 space-y-8">
        ...
        <section>
          ...
          <div className="grid grid-cols-3 gap-4">
            ...
            <SummaryCard label="Saídas" value={formatBRL(outflow)} color="text-red-500" />
            <SummaryCard
              label="Saldo"
        ...
        <section>
          ...
          {!links || links.length === 0 ? (
            <EmptyState
              ...
              description="Clique em "Conectar banco" para importar suas transações."
            />
          ) : (
                ...
                >
                  <div className="min-w-0">
                    ...
                    <div className="mt-1 text-xs text-muted-foreground">
                      {l.last_sync_at
                        ? `Sync ${new Date(l.last_sync_at).toLocaleString("pt-BR", { dateStyle: "short", timeStyle: "short" })}`
                        : "Nunca sincronizado"}
                    </div>
                    {l.last_error && (
        ...
        <section>
          ...
          {!txs || txs.length === 0 ? (
            <EmptyState
              ...
              description="Após conectar uma conta, clique no ícone de sync para importar."
            />
          ) : (
...
function SummaryCard({
  ...
  return (
    <div
      className={`rounded-xl border p-4 ${highlight ? "border-primary/20 bg-primary/5" : "border-border bg-background"}`}
    >
...
function StatusDot({ status }: { status: string }) {
  const color =
    status === "valid" ? "bg-emerald-500" : status === "error" ? "bg-red-500" : "bg-yellow-500";
